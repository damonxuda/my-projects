...
export async function getUserAccessibleFolders(user) {
  try {
    ...
    console.log("返回所有文件夹（前端控制显示）:", allFolders);
    return allFolders;
  } catch (error) {
...
export function isAdmin(user) {
  // 优先使用用户ID进行判断，因为JWT中有user ID但可能没有email
  ...
  const isAdminById = adminUserIds.length > 0 && adminUserIds.includes(user.id);
  const isAdminByEmail = adminEmails.length > 0 && adminEmails.includes(user.emailAddresses?.[0]?.emailAddress);

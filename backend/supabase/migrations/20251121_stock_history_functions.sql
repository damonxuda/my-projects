-- 为美股交易创建历史数据查询函数
-- 直接使用表中存储的 round_number 列

-- 1. 最近5个交易日历史（约70轮，每天14轮）
-- 返回最近70轮的数据
CREATE OR REPLACE FUNCTION get_stock_portfolio_history_24h()
RETURNS TABLE (
  agent_name TEXT,
  total_value NUMERIC,
  round_number INT,
  created_at TIMESTAMPTZ
) AS $$
BEGIN
  RETURN QUERY
  WITH ranked_portfolios AS (
    SELECT
      p.agent_name,
      p.total_value,
      p.created_at,
      p.round_number,
      -- 在每个轮次内取最新的记录（以防同一轮次有多条记录）
      ROW_NUMBER() OVER (
        PARTITION BY p.agent_name, p.round_number
        ORDER BY p.created_at DESC
      ) AS rn
    FROM stock_trading_portfolios p
    WHERE p.round_number IS NOT NULL
  ),
  -- 找出最大轮次
  max_round AS (
    SELECT MAX(round_number) as max_r FROM ranked_portfolios
  )
  -- 返回最近70轮数据（约5个交易日）
  SELECT
    rp.agent_name,
    rp.total_value,
    rp.round_number,
    rp.created_at
  FROM ranked_portfolios rp, max_round mr
  WHERE rp.rn = 1
    AND rp.round_number > mr.max_r - 70
  ORDER BY rp.round_number ASC, rp.agent_name;
END;
$$ LANGUAGE plpgsql;

-- 2. 7天历史（美股不使用，保留空函数以兼容API）
CREATE OR REPLACE FUNCTION get_stock_portfolio_history_7d()
RETURNS TABLE (
  agent_name TEXT,
  total_value NUMERIC,
  round_number INT,
  created_at TIMESTAMPTZ
) AS $$
BEGIN
  -- 美股不提供7天视图，返回空结果
  RETURN QUERY
  SELECT
    NULL::TEXT as agent_name,
    NULL::NUMERIC as total_value,
    NULL::INT as round_number,
    NULL::TIMESTAMPTZ as created_at
  WHERE FALSE;
END;
$$ LANGUAGE plpgsql;

-- 3. 30天历史（每天只显示收盘数据 = 当天最后一轮）
CREATE OR REPLACE FUNCTION get_stock_portfolio_history_30d()
RETURNS TABLE (
  agent_name TEXT,
  total_value NUMERIC,
  round_number INT,
  created_at TIMESTAMPTZ
) AS $$
BEGIN
  RETURN QUERY
  WITH ranked_portfolios AS (
    SELECT
      p.agent_name,
      p.total_value,
      p.created_at,
      p.round_number,
      -- 按天分组（UTC日期）
      DATE(p.created_at) as trading_date,
      -- 在每天内按轮次排序，取最大的（最后一轮 = 收盘）
      ROW_NUMBER() OVER (
        PARTITION BY p.agent_name, DATE(p.created_at)
        ORDER BY p.created_at DESC
      ) AS daily_rn
    FROM stock_trading_portfolios p
    WHERE p.created_at >= NOW() - INTERVAL '30 days'
      AND p.round_number IS NOT NULL
  )
  -- 每天只取最后一轮（收盘价）
  SELECT
    rp.agent_name,
    rp.total_value,
    rp.round_number,
    rp.created_at
  FROM ranked_portfolios rp
  WHERE rp.daily_rn = 1
  ORDER BY rp.round_number ASC, rp.agent_name;
END;
$$ LANGUAGE plpgsql;
